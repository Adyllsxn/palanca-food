...
export default function Dashboard() {
  ...
  return (
    <div className="h-screen flex bg-black">
      <aside className={`h-full bg-[#0a0a0a] flex flex-col transition-all duration-300 ${sidebarCollapsed ? 'w-16' : 'w-64'} rounded-r-[60px]`}>
        <div className="h-16 flex items-center justify-between px-3">
          ...
          <button
            ...
            className="hidden lg:block p-1.5 hover:bg-gray-800 rounded-lg transition cursor-pointer ml-auto"
          >
            {sidebarCollapsed ? <HiOutlineBars3 className="w-4 h-4 text-gray-400" /> : <HiOutlineChevronLeft className="w-4 h-4 text-gray-400" />}
          </button>
        </div>
        ...
        <nav className="flex-1 py-8">
          <ul className={`space-y-2 ${sidebarCollapsed ? 'px-2' : 'px-3'}`}>
            ...
            <li>
              <button
                onClick={() => { setCurrentView("groups"); setMobileMenuOpen(false); }}
        ...
        <div className="p-3 pb-6">
          <div className="relative">
            ...
            {showUserMenu && (
              <div className={`absolute ${sidebarCollapsed ? 'left-full top-0 ml-2' : 'bottom-full left-0 right-0 mb-2'} bg-gray-900 border border-gray-800 rounded-xl shadow-2xl`}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        {/* Background com gradiente diagonal - da direita para baixo */}
        ...
        <header className="h-16 bg-black/50 backdrop-blur-md flex items-center justify-between px-4 relative z-10 border-b border-gray-800/50">
          <button onClick={() => setMobileMenuOpen(true)} className="lg:hidden text-white cursor-pointer">
          ...
          <div className="flex items-center gap-2 ml-auto">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search..."
                className="w-64 pl-9 pr-4 py-1.5 bg-gray-900/80 border border-gray-700 rounded-full text-white text-sm placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-[#7C5CFF]"
              />
            </div>
        ...
        <div className="flex-1 overflow-y-auto relative z-10">
          {currentView === "home" && (
            <div className="flex flex-col items-center justify-center h-full text-center p-4">
              ...
              <h1 className="text-4xl md:text-5xl font-bold text-white mb-3">Welcome to Chatnal</h1>
              <p className="text-gray-400 text-lg max-w-md">
                Connect with communities, join groups, and start chatting in real-time.
              </p>
              <button
                onClick={() => setCurrentView("chats")}
                className="mt-8 px-8 py-3 bg-gradient-to-r from-[#7C5CFF] to-[#5B3DF5] rounded-full text-white font-medium hover:shadow-lg hover:shadow-[#7C5CFF]/25 transition cursor-pointer"
              >
                Start Messaging
              </button>
            </div>
          ...
          {currentView === "chats" && (
            <div className="flex h-full">
                  ...
                  <div className="space-y-2">
                    {chats.map((chat) => (
                      ...
                      >
                        ...
                        {chat.unread > 0 && (
                          <span className="w-5 h-5 bg-[#7C5CFF] text-white text-xs rounded-full flex items-center justify-center">
                            {chat.unread}
                          </span>
                        )}
                      </div>
              ...
              <div className="flex-1 flex flex-col">
                {selectedChat ? (
                  <>
                    <div className="h-16 flex items-center px-4 bg-black/30 backdrop-blur-sm">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3">
                      {messages.map((msg) => (
                        <div key={msg.id} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
                          <div className={`max-w-[70%] px-4 py-2 rounded-2xl ${msg.sender === 'me' ? 'bg-[#7C5CFF] rounded-tr-sm' : 'bg-gray-800 rounded-tl-sm'}`}>
                    ...
                    <div className="p-3 bg-black/30 backdrop-blur-sm">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={handleSend}
                          className="w-9 h-9 bg-gradient-to-r from-[#7C5CFF] to-[#5B3DF5] rounded-full flex items-center justify-center hover:shadow-lg transition cursor-pointer"
                        >
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 bg-black/60 z-40 lg:hidden cursor-pointer" onClick={() => setMobileMenuOpen(false)} />
      )}
    </div>
  );
}